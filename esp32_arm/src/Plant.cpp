...
//จูนตำแหน่งตรงนี้นะ พวกมึง
double offset_z_axis = 0.0;
...
void cover_the_soil(){
  ...
  myservo[1].write(pos_degree[0]);
  moveMM(stepper2,174.0 + offset_z_axis);
  // delay(300);
  for(int i = 0;i < 2;i++) gripper_open_close();
  //ป้องกันชนขอบ
  moveMM(stepper1,30.0);
  moveMM(stepper2,152.0 + offset_z_axis);
  //---------
  moveMM(stepper1,7.0);
  moveMM(stepper2,100.0 + offset_z_axis);
  stepper1.disableOutputs();
  stepper2.disableOutputs();
}
void gripper_open_close(){
...
void goHomeZero(AccelStepper &s1, AccelStepper &s2, long tol_steps) {
  ...
  moveMM(stepper2,35.0 + offset_z_axis);
...
void move_steppers_01() {
  ...
  // anti_vibration(2);
  moveMM(stepper2,166.0 + offset_z_axis);
...
void move_steppers_02() {
  ...
  // myservo[1].write(0);
  moveMM(stepper2,71.0 + offset_z_axis);
  ...
  myservo[1].write(pos_degree[1]);
  delay(300);
  moveMM(stepper2,32.0 + offset_z_axis);//ยกออก จากที่วางต้นกล้า
  stepper1.disableOutputs();
  stepper2.disableOutputs();
}
void move_steppers_03() {
...
void move_driller(){
  ...
  //driller
    ...
    moveMM(stepper2,147.0 + offset_z_axis);//145
    // delay(300); 
    stepper2.setMaxSpeed(7400);
    stepper2.setAcceleration(5500);
    moveMM(stepper2,35.0 + offset_z_axis);
